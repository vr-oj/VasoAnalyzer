...
- Build the onedir bundle: `pyinstaller --noconfirm packaging/VasoAnalyzer.spec` (outputs `dist/VasoAnalyzer/` with `VasoAnalyzer.exe`, icons, and `VasoDocument.ico`).
- Document icon sources live in `resources/icons/`; the PyInstaller spec copies both `.ico` and `.icns` into the dist root for installer consumption.
- Create the installer with `packaging/windows/build_installer.ps1` (requires `iscc` in `PATH`). Output lands in `packaging/windows/output/VasoAnalyzer-Setup.exe`.
...
## macOS (.app bundle)
- Build the app bundle with `pyinstaller --noconfirm packaging/VasoAnalyzer.spec` (produces `dist/VasoAnalyzer */VasoAnalyzer ... .app`).
- Document registration and icon mapping are defined in `packaging/macos/Info.plist` (`UTType com.vasoanalyzer.vaso`, `CFBundleTypeIconFile` `VasoDocument`). PyInstaller pulls `resources/icons/VasoDocument.icns` into the app's `Contents/Resources`.
- App icon comes from `src/vasoanalyzer/VasoAnalyzerIcon.icns` via the spec; document icon from `resources/icons/VasoDocument.icns`.
