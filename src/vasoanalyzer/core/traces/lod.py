...
class LODLevel:
    ...
    def window(self, x0: float, x1: float, margin: int = 1) -> TraceWindow:
        ...
        return TraceWindow(
            ...
            avg_pressure_mean=None
            if self.avg_pressure_mean is None
            else self.avg_pressure_mean[lo:hi],
            avg_pressure_min=None
            if self.avg_pressure_min is None
            else self.avg_pressure_min[lo:hi],
            avg_pressure_max=None
            if self.avg_pressure_max is None
            else self.avg_pressure_max[lo:hi],
            set_pressure_mean=None
            if self.set_pressure_mean is None
            else self.set_pressure_mean[lo:hi],
            set_pressure_min=None
            if self.set_pressure_min is None
            else self.set_pressure_min[lo:hi],
            set_pressure_max=None
            if self.set_pressure_max is None
            else self.set_pressure_max[lo:hi],
        )
