...
"""Bounded LRU cache for QImage snapshots."""
...
def qimage_cache_key(frame_index: int, rotation_deg: int = 0) -> tuple[int, int]:
    """Return a cache key for a frame index + rotation."""
    return (int(frame_index), int(rotation_deg) % 360)
...
class QImageLruCache:
    ...
    @classmethod
    def from_env(cls) -> QImageLruCache | None:
