"""Stacked channel track primitives."""
...
class ChannelTrack:
    ...
    def _compute_padded_limits(self, *, margin: Optional[float] = None) -> Optional[Tuple[float, float]]:
        ...
        if not math.isfinite(ymin) or not math.isfinite(ymax):
            return None
        active_margin = self._auto_margin if margin is None else float(margin)
        if math.isclose(ymin, ymax):
            ...
            span = ymax - ymin
            pad = span * active_margin
            ymin -= pad
            ymax += pad
        return ymin, ymax

    def _apply_auto_y(self, span_changed: bool) -> None:
        if not self.ax.get_autoscaley_on():
            self._sticky_ylim = None
            return
        padded = self._compute_padded_limits()
        if padded is None:
            return
        if span_changed or self._sticky_ylim is None:
            self._sticky_ylim = padded
        else:
            current_min, current_max = self._sticky_ylim
            new_min = min(current_min, padded[0])
            new_max = max(current_max, padded[1])
